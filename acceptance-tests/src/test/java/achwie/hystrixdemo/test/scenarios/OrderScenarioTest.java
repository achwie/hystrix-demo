...
public class OrderScenarioTest extends ScenarioTest<GivenStage, WhenOnCatalogPageStage, ThenStage> {
  ...
  public void logged_in_customer_should_be_able_to_place_an_order() {
    given().user_is_logged_in_as(USER_TEST).and().user_is_on_catalog_page();
    when().user_adds_item_$_to_cart_of_quantity_$(PRODUCT_ID_BOOK, 1).and().user_opens_cart_page().and().user_follows_shipping_address_link().and()
